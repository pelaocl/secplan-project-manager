...
import { FormOptionsResponse, ProgramaOption, UserOption, DEFAULT_TIPO_MONEDA } from '../types';
...
function ProjectForm({ isLoading = false, lookupOptions, isEditMode = false, control, errors, watch, setValue }: ProjectFormProps) {
  ...
  // useEffect Corregido para filtrar programas
  useEffect(() => {
    ...
    // Resetea programaId SOLO si es necesario
    ...
    if (currentProgramaId && !isCurrentProgramaValid) {
        setValue('programaId', '', { shouldValidate: true, shouldDirty: true, shouldTouch: true });
    } else if (!selectedLineaId && currentProgramaId) {
        setValue('programaId', '', { shouldValidate: true, shouldDirty: true, shouldTouch: true });
    }
    // Dependencias correctas para evitar bucles innecesarios
  ...
  if (!lookupOptions) {
      ...
      return (
          <Box display="flex" justifyContent="center" p={3}>
              <CircularProgress />
              <Typography ml={2}>Cargando opciones...</Typography>
          </Box>
      );
  }

  // Renderizado con Grid v1 (los warnings volverán, pero es estable)
  return (
    <Grid container spacing={3}> {/* Contenedor Grid Principal v1 */}

      {/* --- Sección Info Básica --- */}
      <Grid item xs={12}>
          <Paper elevation={2} sx={{ p: 3 }}>
              <Typography variant="h6" gutterBottom>Información Básica</Typography>
              {/* --- AHORA SÍ: TODOS LOS CAMPOS DENTRO DEL GRID ANIDADO --- */}
              <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={4}><Controller name="ano" control={control} render={({ field }) => ( <TextField {...field} label="Año Iniciativa" type="number" fullWidth error={!!errors.ano} helperText={errors.ano?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  ...
                  <Grid item xs={12} md={8}><Controller name="direccion" control={control} render={({ field }) => ( <TextField {...field} label="Dirección" fullWidth error={!!errors.direccion} helperText={errors.direccion?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  <Grid item xs={12} sm={6} md={4}><Controller name="superficieTerreno" control={control} render={({ field }) => ( <TextField {...field} label="Superficie Terreno (m²)" type="number" inputProps={{ step: "any" }} fullWidth error={!!errors.superficieTerreno} helperText={errors.superficieTerreno?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  ...
                  <Grid item xs={12}><Controller name="descripcion" control={control} render={({ field }) => ( <TextField {...field} label="Descripción (Interna)" fullWidth multiline rows={3} error={!!errors.descripcion} helperText={errors.descripcion?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
      ...
      <Grid item xs={12}>
          <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
              ...
              <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}><Controller name="colaboradoresIds" control={control} render={({ field }) => ( <Autocomplete multiple options={lookupOptions.usuarios} getOptionLabel={(o) => `${o.name || '?'} (${o.email})`} value={lookupOptions.usuarios.filter(u => field.value?.includes(u.id)) ?? []} onChange={(_, v) => field.onChange(v ? v.map(u => u.id) : [])} isOptionEqualToValue={(o, v) => o.id === v.id} renderInput={(params) => ( <TextField {...params} label="Colaboradores" error={!!errors.colaboradoresIds} helperText={errors.colaboradoresIds?.message}/> )}
                   renderTags={(value: readonly UserOption[], getTagProps) => // Fix de key mantenido
          ...
          <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
              <Typography variant="h6" gutterBottom>Información Financiera (Interna)</Typography>
               <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6} md={4}><Controller name="monto" control={control} render={({ field }) => ( <TextField {...field} label="Monto Referencial" type="number" inputProps={{ step: "any" }} fullWidth error={!!errors.monto} helperText={errors.monto?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  ...
                  <Grid item xs={12} sm={6} md={4}><Controller name="montoAdjudicado" control={control} render={({ field }) => ( <TextField {...field} label="Monto Adjudicado" type="number" inputProps={{ step: "any" }} fullWidth error={!!errors.montoAdjudicado} helperText={errors.montoAdjudicado?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  <Grid item xs={12} sm={6} md={4}><Controller name="codigoExpediente" control={control} render={({ field }) => ( <TextField {...field} label="Código Expediente" fullWidth error={!!errors.codigoExpediente} helperText={errors.codigoExpediente?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
                  ...
                  <Grid item xs={12} sm={6} md={4}><Controller name="codigoLicitacion" control={control} render={({ field }) => ( <TextField {...field} label="Código Licitación" fullWidth error={!!errors.codigoLicitacion} helperText={errors.codigoLicitacion?.message} disabled={isLoading} variant="outlined"/> )}/></Grid>
               </Grid>
          </Paper>
      </Grid>

      {/* El botón de submit se renderiza en la página padre */}

    </Grid> // Contenedor Grid Principal v1
  );
}
