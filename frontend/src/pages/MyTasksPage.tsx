...
type TaskFilter = 'active' | 'overdue' | 'completed';
...
function MyTasksPage() {
    ...
    // Estados para los filtros y vistas
    ...
    // Memoizar la lista de proyectos únicos para el dropdown
    ...
    // Lógica para filtrar las tareas en el frontend
    const filteredTasks = useMemo(() => {
        const now = new Date();
        now.setHours(0, 0, 0, 0);

        return masterTaskList.filter(task => {
            // Filtro por estado (Activas, Vencidas, Completadas)
            let statusMatch = false;
            switch (activeFilter) {
                case 'overdue':
                    statusMatch = !!task.fechaPlazo && isBefore(parseISO(task.fechaPlazo.toString()), now) && task.estado !== EstadoTarea.COMPLETADA && task.estado !== EstadoTarea.CANCELADA;
                    break;
                case 'completed':
                    statusMatch = task.estado === EstadoTarea.COMPLETADA;
                    break;
                case 'active':
                default:
                    statusMatch = task.estado !== EstadoTarea.COMPLETADA && task.estado !== EstadoTarea.CANCELADA;
                    break;
            }
            return statusMatch;
        });
    }, [masterTaskList, activeFilter]);
    ...
    const handleViewDetails = async (task: Task) => {
        if (!task.proyecto?.id) {
            setError("No se puede abrir la tarea porque falta la información del proyecto.");
            return;
        }
    ...
    const renderContent = () => {
        if (loading) {
            return (
                <Box display="flex" justifyContent="center" alignItems="center" sx={{ mt: 4, height: '50vh' }}>
                    <CircularProgress />
                    <Typography sx={{ ml: 2 }}>Cargando tus tareas...</Typography>
                </Box>
            );
        }

        if (error) {
            return ( <Alert severity="error" sx={{ mt: 2 }}>Error al cargar las tareas: {error}</Alert> );
        }
        
        if (filteredTasks.length === 0) {
            let message = "No hay tareas que coincidan con los filtros actuales.";
            if (activeFilter === 'active' && !debouncedSearchTerm && !selectedProjectId) {
                message = "¡Felicidades! No tienes tareas activas.";
            }
            return (
        ...
        switch (currentView) {
            case 'kanban':
                return <KanbanView tasks={filteredTasks} onTaskStatusChange={handleStatusChange} />;
            case 'calendar':
                return (
                    <CalendarView 
                        tasks={filteredTasks} 
                ...
                return (
                    <Stack spacing={1.5} sx={{ mt: 2 }}>
                        {filteredTasks.map((task) => (
                            <TaskListItemCard key={task.id} task={task} onStatusChange={handleStatusChange} onViewDetails={handleViewDetails} />
                        ))}
                    </Stack>
                );
        }
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            {selectedTask && (
                <TaskDetailModal task={selectedTask} open={!!selectedTask} onClose={handleCloseModal} />
            )}

            {loadingModal && (
                 <CircularProgress sx={{ position: 'fixed', top: '50%', left: '50%', zIndex: (theme) => theme.zIndex.drawer + 2 }} />
            )}
        </Container>
    );
}
