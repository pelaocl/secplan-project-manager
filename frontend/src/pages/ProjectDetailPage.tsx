// ========================================================================
// INICIO: Contenido v12 para ProjectDetailPage.tsx (Corrige Banner con Box)
// ========================================================================
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import {
    Container, Box, Typography, CircularProgress, Alert, Paper, Grid, Chip, Divider, Button, Stack, Tooltip, IconButton, SvgIcon, useTheme // <--- Importa useTheme
} from '@mui/material';
// --- Iconos ---
import EditIcon from '@mui/icons-material/Edit'; /* ... (resto de imports de iconos) ... */
...
// import theme from '../theme/theme'; // Ya no necesitamos importar el theme directamente aquí si usamos useTheme
...
function ProjectDetailPage() {
    ...
    console.log('VALOR THEME EN DETALLE:', theme.shape.borderRadius); // <-- Añade este log
    ...
    return (
        <Container maxWidth="lg" sx={{ mt: 2, mb: 4 }}>
            ...
            {/* 1. Banner con Mapa y Acciones - USA BOX EN LUGAR DE PAPER */}
            <Box
                sx={{
                    // Estilos que antes daba Paper elevation={3}
                    boxShadow: theme.shadows[3],
                    // Estilos que definimos antes
                    height: { xs: '250px', sm: '300px', md: '350px' },
                    position: 'relative',
                    overflow: 'hidden', // Mantenemos overflow hidden
                    mb: 3,
                    // WORKAROUND: Corrige bug en MUIv5/React19 donde theme.shape.borderRadius (12)
                    // se renderiza como 144px en este Box vía sx. Dividir por 10 resulta
                    // en un valor visualmente correcto (~14.4px).
                    // REVISAR y ELIMINAR esta división al actualizar MUI/React/Emotion.
                    borderRadius: theme.shape.borderRadius / 10,
                }}
            >
                 {/* Contenido interno del banner (mapa, overlay, texto, botones) no cambia */}
            ...
            <Grid container spacing={3}>
                 ...
                 {isAuthenticated && (
                    <Grid item xs={12}>
                        {/* Este Paper ahora SÍ recibe los estilos del tema */}
                 ...
                 <Grid item xs={12}>
                    {/* Este Paper toma estilos del tema */}
                    <Paper elevation={2} sx={{ p: { xs: 2, md: 3 } }}>
                        ...
                        <Grid container spacing={3} alignItems="flex-start">
                            ...
                            {/* ... resto de IconDetailItems para Info Básica ... */}
                 ...
                 {/* 4. Sección Descripción (Full Width, si existe) */}
                 {project.descripcion && ( <Grid item xs={12}> <Paper elevation={2} sx={{ p: { xs: 2, md: 3 } }}> {/* ... Contenido ... */} <Typography variant="h6" gutterBottom>Descripción (Interna)</Typography> <Divider sx={{ mb: 2 }} /> <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>{project.descripcion}</Typography> </Paper> </Grid> )}

                 {/* 5. Sección Ubicación y Superficies (Full Width) */}
                 <Grid item xs={12}>
                     <Paper elevation={2} sx={{ p: { xs: 2, md: 3 } }}> {/* Padding */}
                ...
                {/* --- Grupo Lado a Lado: Financiamiento y Estado/Fechas (70/30) --- */}
                {isAuthenticated && (project.montoAdjudicado != null || project.lineaFinanciamiento || project.codigoExpediente || project.fechaPostulacion || project.proyectoPriorizado != null || project.createdAt) && (
                    <>
                         ...
                         <Grid item xs={12} md={8}>
                             <Paper elevation={2} sx={{ p: { xs: 2, md: 3 }, height: '100%' }}> {/* Padding */}
...
// ========================================================================
// FIN: Contenido v12 para ProjectDetailPage.tsx (Corrige Banner con Box)
// ========================================================================