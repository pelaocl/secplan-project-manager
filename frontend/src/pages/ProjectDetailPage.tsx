...
import {
    Container, Box, Typography, CircularProgress, Alert, Paper, Grid, Chip, Divider, Button, Stack, Tooltip, IconButton, SvgIcon, useTheme
} from '@mui/material';
// --- Iconos ---
...
import SectionPaper from '../components/layout/SectionPaper'; // Usa SectionPaper
...
function ProjectDetailPage() {
    ...
    if (error) { /* ... */ return ( <Container maxWidth="md"><Alert severity="error" sx={{ mt: 4 }}>{error}</Alert><Button startIcon={<ArrowBackIcon />} sx={{mt: 2}} onClick={() => navigate('/')}> Volver al Listado </Button></Container> ); }
    if (!project) { /* ... */ return ( <Container maxWidth="md"><Typography sx={{ mt: 4 }}>No se encontró el proyecto.</Typography><Button startIcon={<ArrowBackIcon />} sx={{mt: 2}} onClick={() => navigate('/')}> Volver al Listado </Button></Container> ); }
        ...
        <Container maxWidth="lg" sx={{ mt: 2, mb: 4 }}>
            ...
            <Box
                sx={{
                    ...
                    overflow: 'hidden',
                    mb: 3,
                    // Workaround para bug de redondeo (ajusta el divisor si es necesario)
                    borderRadius: (theme.shape.borderRadius || 12) / 10, // Divide por 10 el valor del tema
                }}
            >
                 {/* Mapa */}
                 ...
                 <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 3, p: { xs: 2, md: 3 }, color: 'common.white', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 2 }}>
                     ...
                     {/* Estilo específico del Chip restaurado */}
                     <Chip
                        label={project.codigoUnico || '?'}
                        size="medium"
                        sx={{
                            backgroundColor: project.tipologia?.colorChip || 'rgba(255,255,255,0.3)',
                            color: '#fff',
                            fontSize: '1.4rem',
                            fontWeight: 'bold',
                 ...
                 <Box sx={{ position: 'absolute', top: 0, left: 35, right: 0, zIndex: 3, p: { xs: 1, sm: 2 }, display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      {isAuthenticated && (
                          <Stack direction="row" spacing={1}>
                              {/* Estilo específico del botón Imprimir restaurado */}
                              ...
                              {/* Botón Editar usa color="primary" del tema, no necesita sx complejo */}
            ...
            {/* Contenedor Grid Principal para las secciones (Sin cambios) */}
            <Grid container spacing={3}>
                 {/* 2. Bitácora (Usa SectionPaper) */}
